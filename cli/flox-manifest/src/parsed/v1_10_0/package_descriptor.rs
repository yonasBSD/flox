...
impl SetOutputs for ManifestPackageDescriptor {
    fn set_outputs_to_all(&mut self) {
        match self {
            ManifestPackageDescriptor::Catalog(catalog_pkg) => catalog_pkg.set_outputs_to_all(),
            ManifestPackageDescriptor::FlakeRef(flake_pkg) => flake_pkg.set_outputs_to_all(),
...
impl From<v1::ManifestPackageDescriptor> for ManifestPackageDescriptor {
    fn from(value: v1::ManifestPackageDescriptor) -> Self {
        match value {
            v1::ManifestPackageDescriptor::Catalog(old) => {
                let new = PackageDescriptorCatalog {
                    pkg_path: old.pkg_path,
                    pkg_group: old.pkg_group,
                    priority: old.priority,
                    version: old.version,
                    systems: old.systems,
                    outputs: None,
                };
                ManifestPackageDescriptor::Catalog(new)
            },
            v1::ManifestPackageDescriptor::FlakeRef(old) => {
                let new = PackageDescriptorFlake {
                    flake: old.flake,
                    priority: old.priority,
                    systems: old.systems,
                    outputs: None,
                };
                ManifestPackageDescriptor::FlakeRef(new)
            },
            v1::ManifestPackageDescriptor::StorePath(old) => {
